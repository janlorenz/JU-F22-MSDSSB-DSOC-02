...
To do this, you need to tell GitHub that you are authorized to do this, and to that end you need to make a Personal Access Token (PAT) in your GitHub account and make this available to RStudio and git on your local machine.

Fortunately, there are some tools in R to help you.
Read more about PATs and how to use them in "Happy Git with R" Chapter 9 <https://happygitwithr.com/https-pat.html> (in particular the TL;DR).
If you have installed the packages `usethis` and `gitcreds` thisss can be done by two commands in the console:
...
For today, you can also go the fast way and click "Generate token".
Use the clipboard icon 📋 to copy the PAT.
Go back to RStudio and do:
...
That should be it and you do not need to repeat these steps until the PAT expires. 

Now you can push your commit to GitHub.
