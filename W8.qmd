...
who |> filter(Date_reported > '2020-02-01', Date_reported < '2020-07-01', 
              Country == "Germany") |>
 mutate(Smooth3 = (New_cases + lag(New_cases) + lag(New_cases,2))/3,
        Smooth7 = (New_cases + lag(New_cases) + lag(New_cases,2) + lag(New_cases,3) + lag(New_cases,4) + lag(New_cases,5) + lag(New_cases,6))/7,
        Smooth7 = (New_cases + lag(New_cases) + lag(New_cases,2) + lag(New_cases,3) + lag(New_cases,4) + lag(New_cases,5) + lag(New_cases,6)+ lag(New_cases,7) + lag(New_cases,8) + lag(New_cases,9))/10
) |> pivot_longer(c(New_cases, Smooth3, Smooth7, Smooth10), names_to = "Smooth",
                  values_to = "Cases") |> 
 filter(Date_reported > '2020-03-01', Date_reported < '2020-05-01') |>
 ggplot(aes(Date_reported, Cases, color = Smooth)) + geom_line()
```
