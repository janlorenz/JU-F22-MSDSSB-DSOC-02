...
- select 5 attitude variabels and `ntry` with `r ess$cntry |> unique() |> length()` countries: `r ess$cntry |> unique()`
...
The RMSE of `r rmse(ess_train_pred3,truth = euftf,estimate = .pred)$.estimate` shows how much predicted values deviate from the true value on average. 
...
- For the logistic regression has two performacne measures as default: **AUC** (area under the ROC-curve) and **accuracy**. 
- The model using the two variables about their bills performs better than the model using body mass and flipper length. 



## Accuracy for classifiers {.smaller}

All metrics for binary classifiers derive from the *confusion matrix*. 
...
- For logistic regression: 
  - Accuracy relies on a decision for a threshold (default 50%) and has an easy direct interpretation.
  - AUC is the area under the ROC-curve. It is derived sweeping over all possible thresholds. 
...
```{r}
#| echo: true
peng_fit1_rs |> collect_metrics(summarize = FALSE) |> arrange(.metric)
...
peng_test_pred1 |> accuracy(truth = sex, estimate = .pred_class_0.5)
peng_test_pred1 |> accuracy(truth = sex, estimate = .pred_class_0.55)
peng_test_pred1 |> accuracy(truth = sex, estimate = .pred_class_0.45)
...
```{r}
#| echo: true
peng_fit2_rs |> collect_metrics(summarize = FALSE) |> arrange(.metric)
...
peng_test_pred2 |> accuracy(truth = sex, estimate = .pred_class_0.5)
peng_test_pred2 |> accuracy(truth = sex, estimate = .pred_class_0.55)
peng_test_pred2 |> accuracy(truth = sex, estimate = .pred_class_0.45)
...
- How many chips? [62]{.fragment style='color:red;'}
...
- How many draws?  [62]{.fragment style='color:red;'}
...
This is the fraction of simulations where the p-value was below `r 3/62`. 
